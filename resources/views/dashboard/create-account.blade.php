                            ...
                            <div x-show="currentStep === 8 && isMLBB" x-cloak x-transition.opacity.duration.200ms x-data="listingPreview()" x-init="init()">
                                ...
                                <div id="listingPoster" class="listing-poster" :class="isPremiumLayout ? 'is-premium' : 'is-basic'">
                                    ...
                                    <div class="lp-stats">
                                        ...
                                        <div class="lp-stat lp-stat-rank">
        ...
        function highlightedSkinsPicker() {
            return {
                ...
                init() {
                    ...
                    this.$watch('currentStep', (step) => {
                        if (step === 4) {
                            ...
                            this.enrichSelectedSkins().then(() => {
                                this.updateHiddenInputs();
                            });
                        }
                    });
                    Promise.all([this.loadHeroes(), this.loadSkinIdMap()])
                        .then(() => this.enrichSelectedSkins())
                        .finally(() => {
                            this.updateHiddenInputs();
                ...
                async resolveSkinImages(ids) {
                    ...
                    try {
                        const response = await fetch(`/api/mlbb/skins/resolve?ids=${unique.join(',')}`, {
                            credentials: 'same-origin',
                            headers: { Accept: 'application/json' },
                        });
                        ...
                        for (const item of this.selectedSkins) {
                            const match = byId[Number(item.id)];
                            if (!match) continue;
                            item.image_url = match.image_url || match.thumbnail_url || item.image_url;
                            item.thumbnail_url = match.thumbnail_url || item.thumbnail_url || null;
                            item.rarity = item.rarity || match.rarity || 'Skin';
                            item.tags = item.tags?.length ? item.tags : (match.tags || []);
                            item.painted = item.painted ?? match.painted ?? false;
                        }
                    } catch (error) {
                        console.error('Skin resolve failed', error);
                    }
                },
                restoreSelections() {
                ...
                async enrichSelectedSkins() {
                    ...
                    const missingIds = this.selectedSkins
                        .filter((item) => !item.image_url && !item.thumbnail_url && item.id)
                        .map((item) => item.id);
        ...
        function listingPreview() {
            return {
                ...
                async resolvePosterImageDataUrl(img, src) {
                    ...
                    try {
                        const fetchUrl = src.startsWith('http') ? src : new URL(src, window.location.origin).href;
                ...
                registerPosterImageMapEntry(map, key, dataUrl) {
                    if (!key || !dataUrl) return;
                    map.set(key, dataUrl);
                    try {
                        const parsed = new URL(key, window.location.origin);
                        map.set(parsed.href, dataUrl);
                        const relative = parsed.pathname + parsed.search;
                        if (relative) {
                            map.set(relative, dataUrl);
                        }
                    } catch {
                        // ignore invalid URLs
                    }
                },
                lookupPosterImageDataUrl(imageMap, src) {
                    if (!src) return null;
                    if (imageMap.has(src)) {
                    ...
                    try {
                        ...
                        if (relative && imageMap.has(relative)) {
                            return imageMap.get(relative);
                        }
                    } catch {
                        // ignore invalid URLs
                    }
                ...
                async buildPosterImageDataMap(posterEl) {
                    ...
                    for (const img of images) {
                        ...
                        try {
                            const parsed = new URL(key, window.location.origin);
                            aliases.push(parsed.href, parsed.pathname + parsed.search);
                        } catch {
                            // ignore
                        }
                        if (aliases.some((alias) => alias && map.has(alias))) {
                            continue;
                        }
                        ...
                        if (dataUrl) {
                            aliases.forEach((alias) => {
                                if (alias) {
                                    this.registerPosterImageMapEntry(map, alias, dataUrl);
                                }
                            });
                        }
                ...
                readFrameState(key, img) {
                    ...
                    if (stored && (stored.adjusted || stored.scale || stored.coverScale)) {
                        return {
                            x: Number(stored.x) || 0,
                            y: Number(stored.y) || 0,
                            scale: Number(stored.scale) || Number(stored.coverScale) || 1,
                        };
                    }
                ...
                rasterizeFrameFromComputedStyle(img, vw, vh, background = '#111111') {
                    ...
                    layerCtx.drawImage(img, dx, dy, dw, dh);
                ...
                async ensureFrameImagesInMap(posterEl, imageMap) {
                    for (const img of posterEl.querySelectorAll('[data-frame-key] img')) {
                        const candidates = [
                            img.dataset.posterSrc,
                            img.currentSrc,
                            img.src,
                        ...
                        for (const original of candidates) {
                            if (this.lookupPosterImageDataUrl(imageMap, original)) {
                                found = true;
                                break;
                            }
                        }
                        ...
                        for (const original of candidates) {
                            const dataUrl = await this.resolvePosterImageDataUrl(img, original);
                            if (dataUrl) {
                                this.registerPosterImageMapEntry(imageMap, original, dataUrl);
                                break;
                            }
                        }
                        await this.yieldToBrowser(0);
                ...
                savePosterImageStates(posterEl) {
                    return [...posterEl.querySelectorAll('img')].map((img) => ({
                        img,
                        src: img.getAttribute('src') || '',
                        style: img.getAttribute('style') || '',
                    }));
                },
                restorePosterImageStates(states) {
                    (states || []).forEach(({ img, src, style }) => {
                        if (!img) return;
                        if (src) {
                ...
                applyAllPosterImageDataMap(posterEl, imageMap) {
                    posterEl.querySelectorAll('img').forEach((img) => {
                ...
                async bakePosterFrames(posterEl, imageMap) {
                    ...
                    for (const viewport of posterEl.querySelectorAll('[data-frame-key]')) {
                        ...
                        let inlined = null;
                        for (const original of candidates) {
                            inlined = this.lookupPosterImageDataUrl(imageMap, original);
                            if (inlined) break;
                        }
                        if (!inlined) continue;
                ...
                async bakeLivePosterFramesInPlace(posterEl) {
                    for (const viewport of posterEl.querySelectorAll('[data-frame-key]')) {
                        const key = viewport.getAttribute('data-frame-key');
                        const img = viewport.querySelector('img');
                        if (!key || !img || !img.naturalWidth) continue;

                        const vw = Math.max(1, Math.round(viewport.clientWidth || viewport.offsetWidth));
                        const vh = Math.max(1, Math.round(viewport.clientHeight || viewport.offsetHeight));
                        const background = this.frameViewportBackground(viewport);

                        let dataUrl = this.rasterizeFrameFromComputedStyle(img, vw, vh, background);
                        if (!dataUrl) {
                            const frame = this.readFrameState(key, img);
                ...
                applyRasterizedFrames(posterEl, rasterized, imageMap, liveEl = null) {
                    const liveSources = new Map();
                    if (liveEl) {
                        liveEl.querySelectorAll('[data-frame-key]').forEach((viewport) => {
                            const key = viewport.getAttribute('data-frame-key');
                            const img = viewport.querySelector('img');
                            if (!key || !img) return;
                            liveSources.set(key, [
                    ...
                    posterEl.querySelectorAll('[data-frame-key]').forEach((viewport) => {
                        ...
                        let dataUrl = rasterized.get(key) || null;
                        if (!dataUrl) {
                            const sources = liveSources.get(key) || [
                                viewport.querySelector('img')?.dataset?.posterSrc,
                                viewport.querySelector('img')?.currentSrc,
                                viewport.querySelector('img')?.src,
                                viewport.querySelector('img')?.getAttribute('src'),
                            ].filter(Boolean);
                            for (const original of sources) {
                                dataUrl = this.lookupPosterImageDataUrl(imageMap, original);
                                if (dataUrl) break;
                            }
                        }
                        ...
                        this.flattenCloneFrameImage(newImg);
                        viewport.innerHTML = '';
                ...
                preparePosterCloneForExport(clone, { imageMap, rasterized, live } = {}) {
                    ...
                    this.applyRasterizedFrames(clone, rasterized || new Map(), imageMap || new Map(), live);
                ...
                async exportPosterBlob() {
                    ...
                    this.downloadStatus = 'Inlining skin artwork…';
                    await this.ensureFrameImagesInMap(el, imageMap);
                    if (imageMap.size === 0) {
                    ...
                    if (rasterized.size === 0) {
                        console.warn('Poster export: no baked skin frames, using inline fallbacks');
                    }
