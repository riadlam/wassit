@extends('layouts.app', ['title' => __('privacy.title') . ' | Wassitmarket'])

@section('content')
    @php
        $sections = __('privacy.sections');
    @endphp

    <div class="relative min-h-screen overflow-hidden">
        <div class="absolute inset-0 pointer-events-none">
            <div class="absolute inset-0 bg-gradient-to-b from-[#0f111b] via-[#151721] to-[#1b1a1e]"></div>
            <div class="absolute top-0 left-1/2 h-96 w-96 -translate-x-1/2 rounded-full bg-blue-500/10 blur-3xl"></div>
        </div>

        <div class="relative z-10 px-4 py-16 mx-auto max-w-4xl sm:px-6 sm:py-24 lg:px-8">
            <header class="mb-10 text-center">
                <div class="inline-flex items-center justify-center w-14 h-14 mb-5 text-xl text-blue-400 border rounded-2xl border-blue-400/20 bg-blue-400/10">
                    <i class="fa-solid fa-shield-halved" aria-hidden="true"></i>
                </div>
                <h1 class="text-3xl font-bold tracking-tight text-white sm:text-5xl">
                    {{ __('privacy.title') }}
                </h1>
                <p class="mt-4 text-sm text-gray-400">{{ __('privacy.effective_date') }}</p>
            </header>
                    ...
                    @foreach($sections as $section)
                        <section>
                            ...
                            @if(!empty($section['items']))
                                <ul class="mt-4 space-y-3 text-gray-300 list-disc ps-6 marker:text-blue-400">
