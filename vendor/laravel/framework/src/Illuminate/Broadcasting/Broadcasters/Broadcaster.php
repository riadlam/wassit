...
abstract class Broadcaster implements BroadcasterContract
{
    ...
    {
        \Log::info('verifyUserCanAccessChannel', [
            'channel' => $channel,
            'registered_patterns' => array_keys($this->channels),
        ]);
        
        foreach ($this->channels as $pattern => $callback) {
            $matches = $this->channelNameMatchesPattern($channel, $pattern);
            \Log::info('Pattern match check', [
                'channel' => $channel,
                'pattern' => $pattern,
                'matches' => $matches,
            ]);
